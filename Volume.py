...
print('프론트엔드, 백엔드, db서버 각각 도커화가 가능하다.')
print('백엔드가 컨테이너에서 실행되는 동안 노출되는 포트를 게시하지 않으면 연결할 수 없다.')
print(' dockerfile에 Expose 명령을 통해 포트를 게시해야 한다.')

print('리액트를 도커화 할 때 -it로 인터렉티브 모드로 실행해야한다. 명령을 입력하여 상호작용하는 것을 원하기 때문이다.')
